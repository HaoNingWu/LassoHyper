function [err_I,errest_I,err2_I,LV,LCalfa,Ltime,Calfa,f_name,Finterp]=CX_Interpolationlasso(n,f_idx,lambda,sigma)
% Based on M-function "CX_Interpolation" of Stefano De Marchi and Marco Vianello, we 
% write this function for Lasso hyperinterpolation in a cube
...
scaling=[1,ones(1,n)*sqrt(2)]';  % scaling for yshev polynomials
...
for i = 1:m
    if abs(Calfa(i)) <= lambda
        Calfa(i) = 0;
    elseif Calfa(i)>lambda
        Calfa(i) = Calfa(i)-lambda;
    else
        Calfa(i) = Calfa(i)+lambda;
    end
end
...
% for i = 1:m
%     Calfa(i) = wthresh(Calfa(i),'h',lambda);
% end
